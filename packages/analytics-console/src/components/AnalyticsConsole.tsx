...
export function AnalyticsConsole({
  ...
  // Load the full live detail once. Minute freshness is owned by the bounded
  // tail query below and lifecycle/viewer freshness by the status endpoint;
  // polling the full detail would re-download the complete minute history.
  ...
  const statusQuery = useQuery({
    ...
    enabled: Boolean(targetQueryStreamId && baseDetail),
    staleTime: 30_000,
  ...
  const channelEmotesQuery = useQuery({
    queryKey: ['analytics-console-channel-emotes', channelLogin],
    queryFn: () => getChannelEmoteCatalog(channelLogin),
    // Channel identity is enrichment. Start it only after the core chart is
    // available and cache it across sessions for the same channel.
    enabled: Boolean(channelLogin && chartDetailReady),
    staleTime: 30 * 60_000,
    retry: 1,
  })
  ...
  const detailForRender = useMemo(() => {
    const stagedTopEmotes = summaryQuery.data?.topEmotes?.length
      ? summaryQuery.data.topEmotes
      : channelEmotesQuery.data
    if (!detail || !stagedTopEmotes?.length) return detail
    return { ...detail, topEmotes: stagedTopEmotes }
  }, [channelEmotesQuery.data, detail, summaryQuery.data?.topEmotes])
  ...
  // Auto mode follows the explicit chart focus. Manual overlay selections
  // remain stable because only the `auto` sentinel is resolved by view mode.
  const emotePlotViewMode = viewMode === 'emotes'
    ? 'emotes' as const
    : viewMode === 'spikes'
      ? 'spikes' as const
      : 'overview' as const
  ...
  const toggleSelected = useCallback((key: string) => {
    setEmotePlotSelection((current) =>
      toggleEmotePlotSelection(current, key, detailForRender?.topEmotes ?? [], emotePlotViewMode),
    )
  }, [detailForRender?.topEmotes, emotePlotViewMode])
  ...
  const chartEmoteKeys = useMemo(
    () => resolveChartEmoteKeys(emotePlotSelection, detailForRender?.topEmotes ?? [], emotePlotViewMode),
    [detailForRender?.topEmotes, emotePlotSelection, emotePlotViewMode],
  )
