...
export function portalMinutesToRollups(
  ...
  // them all at the same x, faking a huge spike at minute 0. Keep the LAST row
  // per offset (matches backend consolidate-by-minute last-wins semantics).
  const byOffset = new Map<number, PortalMinutePoint>()
  for (const minute of minutes) {
    byOffset.set(Math.max(0, minute.offsetSeconds), minute)
  }
  const rollups = [...byOffset.values()].map((minute) => {
