...
export function fromHubMoment(moment: FigmaMomentRow & { archiveArtwork?: ArchiveArtwork }): DiscoveryMoment | null {
  ...
  // Relative evidence is only meaningful for the same measured minute as the
  // detector row. Keep same-minute partial evidence for its honest baseline
  // disclosure, but never let a comparison from another minute become this
  // row's headline or count source.
  const comparisonForMoment = comparison && (
    at == null || comparisonAt == null ||
    Math.floor(at / 60_000) === Math.floor(comparisonAt / 60_000)
  ) ? comparison : undefined
  // Keep the headline and counts on the same server-supplied measurement.
  // Detector peaks can retain older counts while the minute rollup is revised.
  const verifiedMinute = at != null && comparisonAt != null
    && Math.floor(at / 60_000) === Math.floor(comparisonAt / 60_000)
    && comparisonForMoment?.evidence?.ircBound === true && comparisonForMoment.evidence.eventRollupAvailable === true
    && [comparisonForMoment.chat.currentPerMin, comparisonForMoment.emotes.currentPerMin]
  ...
  return { key, publicMomentId, login, streamId: moment.streamId, offsetSeconds: moment.offsetSeconds,
    ...
    comparison: comparisonForMoment, reactionSignal: momentReactionSignal(moment.kind), topEmotes: moment.topEmotes,
...
export function fromNewsroomUpdate(story: NewsroomStory, update: NewsroomUpdate): DiscoveryMoment | null {
  ...
  const moment = fromHubMoment({ login: story.login, streamId: story.streamId,
    ...
    profileImageUrl: story.profileImageUrl, category: story.category, comparison: update.comparison,
...
export interface CheckedMomentSource { vodHref: string | null; vodOffsetSeconds?: number; archiveArtwork?: import('./archiveArtwork').ArchiveArtwork; handoffRef?: string; liveHref: string | null; liveExpiresAt?: number; occurrenceAt?: number; reason: string; pendingArchive?: boolean; retryable?: boolean; lookupFailed?: boolean }
...
export async function checkMomentSource(moment: DiscoveryMoment, signal: AbortSignal, refreshHandoff = false): Promise<CheckedMomentSource> {
  const { data } = await apiClient<{ channel?: string; vodId?: string; vodAlignSeconds?: number; vodArtwork?: unknown;
    vodDurationSeconds?: number; vodTiming?: { state?: string; reason?: string };
    handoffRef?: string;
    availability?: { vodId?: string; vodState?: string };
    stream?: { streamId?: string; channel?: string; login?: string; startedAt?: string; vodId?: string; lifecycleState?: string; lifecycleObservedAt?: string } }>(
  ...
  const vodHref = validVodId(vodId) && vodOffsetSeconds !== undefined
    ? buildVodTimestampUrl(vodId, vodOffsetSeconds) : null
  const pendingArchive = !vodHref && ['pending_live', 'resolving'].includes(data.availability?.vodState ?? '')
  const lookupFailed = (suppliedIds.length === 0 || consistentIds)
    && (data.vodTiming?.reason === 'lookup_failed' || data.availability?.vodState === 'request_failed')
  const retryable = !vodHref && (pendingArchive || lookupFailed || (outsideArchive && alignedOffset! >= 0))
  ...
  return { vodHref, vodOffsetSeconds, occurrenceAt, pendingArchive, retryable, lookupFailed, archiveArtwork: vodHref ? verifiedArchiveArtwork(data.vodArtwork, vodId) : undefined,
