...
- [x] **RPR-1** Request contract + performance (R14): request matrix tests; chart migration v2; content-owned poller + `pulseGetCoordinator` coalesce; activation-scoped Full; backfill operation cancellation; real HTTP routing matrix; bundle-budget CI; content watch rebuild; both-host uniqueness E2E. _Accepted:_ master `e13a74e` (PRs #49/#50/#51); force-full CI [`30144042786`](https://github.com/Aron-Chu/streamclone-pulse/actions/runs/30144042786) (`force_full=true`, `e2e_executed=true`, final-gate OK). Prior force-full `30138875909` retained as historical evidence only.
- [x] **RPR-2** Manifest targets (dev/CWS/Edge/Firefox-later) without localhost on store builds (R18); yauzl ZIP byte validation of all archived bytes; store DCE; exact permission allowlists; distinct CWS/Edge ZIP names; portal localhost rewrite to `https://invalid.invalid` + fail-closed scanner; RPR-6 readiness note for sibling `file:` deps. _Accepted:_ master `e13a74e` (PRs #49/#51); force-full CI `30144042786` (CWS+Edge validation ran; artifacts **not uploaded**). RPR-6 remains the public clean-build blocker.
