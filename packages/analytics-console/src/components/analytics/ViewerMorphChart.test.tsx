...
describe('session viewer morph chart', () => {
  ...
  it('measures the rendered plot width instead of treating viewBox units as CSS pixels', async () => {
    const { container } = render(<Harness />)
    const svg = container.querySelector('svg')
    ...
    await waitFor(() => expect(svg.getAttribute('data-viewer-plot-css-width')).toBe('420.48'))
  })
  ...
  it('keeps immutable detail d while hover only changes clip widths and preserves historical line focus', async () => {
    ...
    const clipRects = Array.from(container.querySelectorAll('clipPath rect')) as SVGRectElement[]
    ...
    expect(Array.from(container.querySelectorAll('clipPath rect')).map(rect => rect.getAttribute('width'))).not.toEqual(restWidths)
