...
Naming: **streampulse-backend** = backend engine (Go BFF, ingest) · **Streamclone** = public Twitch watch replica (watch-only after boundary split; NOT the BFF) · **Streamclone Pulse** = this extension repo (owns `@streampulse/*` packages) · **StreamPulse** = this website · Hosted API = `https://api.streampulse.stream`.
...
- [x] MOMENT-102: Stream recap improvements (all-provider emotes, expanded moments list, chart strip)
  - Area: backend / frontend
  - Priority: V2
  - Depends on: DASH-006
  - Files likely touched:
...
2. **INFRA-003** — Hosted-mode environment variables on hosted-production-vps (operator env in private streampulse-ops).
