...
Related code: `@streampulse/pulse-core` (**this repo** `packages/pulse-core/`), **this repo** `src/` (content script, service worker, `ui/Overlay.tsx`), **streampulse-backend** `internal/analytics/extension_api.go` (BFF + Redis cache), `bookmarks.go`, `recap/`.
...
source ownership is complete; **package-distribution acceptance** remains open
until clean-consumer + NOTICE/ZIP gates pass.
...
Canonical contracts: [`contracts/`](./contracts/README.md) (PostHog named as RPR-5 processor; activation pending).

1. Extension diagnostics and product analytics SHALL have **separate**, versioned, **default-off** consent with prominent in-product disclosure before collection. *(Not implemented in the MV3 package.)*
...
4. Privacy / product-support / security reporting channels SHALL be separate once each channel is verified and operational. Unverified addresses MUST NOT be published as active contacts. Verified public mailbox today: **`privacy@streampulse.stream` only**.
...
6. **RPR-5 processor:** product analytics SHALL use **PostHog** via backend aggregates only (no browser SDK, autocapture, replay, or person profiles).
