...
test('channel analytics renders streamclone console by default', async ({ page }) => {
  ...
  await expect(page.locator('.sc-analytics-console').getByRole('button', { name: /Sync chat/i })).toHaveCount(0)
  await expect(page.locator('.sc-analytics-console').getByRole('button', { name: /Re-sync/i })).toHaveCount(0)
...
test('channel analytics figma mode renders session dashboard', async ({ page }) => {
  ...
  await expect(page.getByRole('main', { name: /Analytics for jynxzi/i })).toBeVisible()
  await expect(page.locator('.figma-session')).toBeVisible()
