...
import Changelog from '../src/routes/public/Changelog'
...
describe('public release routes', () => {
  ...
  it('publishes the canonical shipped-release changelog', () => {
    renderRoute(<Changelog />)
    const page = screen.getByTestId('changelog-page')
    expect(within(page).getByRole('heading', { name: /^changelog$/i })).toBeTruthy()
    expect(within(page).getByTestId('release-0.1.0')).toBeTruthy()
    expect(within(page).getByRole('link', { name: /roadmap/i }).getAttribute('href')).toContain('github.com')
  })
})
