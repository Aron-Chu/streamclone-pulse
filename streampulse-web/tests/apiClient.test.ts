...
import { clearBetaKey, setBackendUrlOverride, setBetaKey } from '../src/lib/auth'
...
describe('apiClient', () => {
  ...
  it('rejects a successful response with invalid JSON instead of returning an error-shaped payload', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => new Response('<html>upstream error</html>', { status: 200 })))
    await expect(apiClient('/v1/public/hub')).rejects.toMatchObject({
      kind: 'server', status: 200, code: 'invalid_json_response',
    })
  })
  ...
  it('routes sensitive support submissions through the active backend override', async () => {
    setBackendUrlOverride('https://api.example.test')
    const fetchMock = vi.fn(async (url: RequestInfo, init?: RequestInit) => {
      expect(String(url)).toBe('https://api.example.test/v1/portal/support/cases')
      expect(init?.redirect).toBe('error')
      expect(new Headers(init?.headers).has('X-Streamclone-Beta-Key')).toBe(false)
      return new Response('{"case_id":"one"}')
    })
    ...
    await apiClient('/v1/portal/support/cases', {
      method: 'POST',
      sensitive: true,
      body: { turnstile_token: 'private', description: 'private' },
    })
    expect(fetchMock).toHaveBeenCalledTimes(1)
  })
})
