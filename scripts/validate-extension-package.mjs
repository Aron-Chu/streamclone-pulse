...
async function main() {
  ...
  stageExtensionAttribution(dist)
  const attribution = validateExtensionAttributionBytes(dist)
  if (!attribution.ok) {
    for (const error of attribution.errors) fail(error)
  } else {
  ...
  if (rpr6.length) {
    fail('RPR-6 blocker: sibling file: dependencies remain (clean public source required)')
    for (const hit of rpr6) fail(`  [${hit.source}] ${hit.section} ${hit.name}=${hit.spec}`)
  } else {
    ok('REQUIRED: public-source readiness (no escaping sibling file: deps)')
  }
