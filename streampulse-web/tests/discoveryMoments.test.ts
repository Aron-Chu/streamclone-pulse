...
describe('discovery identity and source actions', () => {
  ...
  it('explains archive publication separately from unknown source identity', async () => {
    ...
    expect(result.pendingArchive).toBe(true)
    expect(result.retryable).toBe(true)
    expect(result.liveHref).toBeNull()
  })
  it('distinguishes transient lookup failure from contradictory archive identities', async () => {
    const data = { channel: 'creator', stream: { streamId: 'stream-a', vodId: '77777' }, vodTiming: { state: 'unavailable', reason: 'lookup_failed' } }
    request.mockResolvedValue({ data })
    const failed = await checkMomentSource(fromHubMoment(row)!, new AbortController().signal)
    expect(failed.lookupFailed).toBe(true)
    expect(failed.retryable).toBe(true)
    expect(failed.vodHref).toBeNull()
    request.mockResolvedValue({ data: { ...data, vodId: '88888' } })
    const conflicting = await checkMomentSource(fromHubMoment(row)!, new AbortController().signal)
    expect(conflicting.lookupFailed).toBe(false)
    expect(conflicting.reason).toContain('conflicting archive identities')
  })
