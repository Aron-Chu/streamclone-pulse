...
export function summarizeActivity(
  ...
  const measuredPoints = chartActivityPoints(points, windowMinutes)
    .filter((point) => !point.synthetic)
  const pointCount = measuredPoints.length
  const nonZeroCount = measuredPoints.filter(activePoint).length
