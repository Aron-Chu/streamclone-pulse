...
export async function inspectZipCentralDirectory(zipPath, limits = ZIP_LIMITS) {
  ...
  try {
    ...
    return {
      entries: [],
      errors: [`zip open failed: ${err instanceof Error ? err.message : String(err)}`],
    }
  }
  const entries = []
  const errors = []
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      zip.on('entry', (entry) => {
        try {
          ...
          errors.push(
            `zip entry rejected: ${entryErr instanceof Error ? entryErr.message : String(entryErr)}`,
          )
          try {
            zip.close()
          } catch {
      ...
      zip.on('error', (err) => {
        errors.push(`zip read failed: ${err instanceof Error ? err.message : String(err)}`)
        try {
          zip.close()
        } catch {
          // ignore
    ...
    errors.push(`zip inspect failed: ${err instanceof Error ? err.message : String(err)}`)
  }
...
export async function extractZipToTemp(zipPath, limits = ZIP_LIMITS) {
  ...
  try {
    ...
    if (/not enough bytes in the stream/i.test(message)) {
      throw new Error(`declared uncompressed-size mismatch: ${message}`)
    }
    throw err
  }
