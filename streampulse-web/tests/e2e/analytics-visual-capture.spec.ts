    ...
    for (const target of TARGETS) {
      test(`${target.slug} @ ${viewport.name}`, async ({ page }, testInfo) => {
        ...
        if (target.slug === 'channel-console') {
          ...
          await expect(page.getByRole('heading', { level: 1, name: /Command center/i })).toBeVisible({ timeout: 15_000 })
          await expect(page.getByText(/Reading Hosted API/i)).toHaveCount(0)
        }
