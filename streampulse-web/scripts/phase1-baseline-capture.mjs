...
async function main() {
  ...
  // Mobile (390×844)
  {
    ...
    if (box) {
      ...
      // Vertical scroll test: swipe downward — should scroll the page if touch-action: pan-y
      await page.evaluate(() => window.scrollTo(0, 0))
      await page.waitForTimeout(200)
      await touch(150, 200, 150, 700)
      await page.waitForTimeout(300)
      const scrollY = await page.evaluate(() => window.scrollY)
      console.log(`  mobile vertical scroll after downward touch swipe: scrollY=${scrollY}`)
      captures.push({ label: 'mobile-after-vertical-scroll', probe: { scrollY } })
