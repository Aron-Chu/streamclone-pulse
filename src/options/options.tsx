  ...
  return (
    <main style={styles.page}>
      ...
      <section style={styles.section}>
        ...
        <label style={styles.checkboxRow}>
          <input
            ...
            onChange={event => {
              ...
              void setAnalyticsConsentGranted(enabled).catch(() => {
                // Storage failure: revert UI and keep in-memory denial.
        ...
        <p style={styles.help}>
          Optional and off by default. Separate from crash diagnostics. When enabled (and only after
          StreamPulse activates the server path), the extension may send fixed aggregate event names
          such as <code>pulse_load_completed</code> and <code>extension_error_shown</code> via the
          StreamPulse backend to PostHog for processing. No channel names, free text, correlation IDs,
          person profiles, or identity. Retention target ~180 days (not yet activated). Withdraw anytime
          by unchecking — new sends stop immediately.
        </p>
      </section>
      ...
      <section style={styles.section}>
        ...
        <label style={styles.checkboxRow}>
          <input
            type="checkbox"
            checked={shareCrashDiagnostics}
            onChange={event => {
              const enabled = event.target.checked
              setShareCrashDiagnostics(enabled)
              void setDiagnosticsConsentEnabled(enabled)
            }}
          />
          Share crash diagnostics
        </label>
        <p style={styles.help}>
          Off by default. When enabled, StreamPulse may send sanitized crash diagnostics
          (fixed enum fields and bounded stack frames only — no raw messages, chat, channel
          logins, stream/VOD IDs, cookies, or debug logs) to the StreamPulse backend, which
          constructs events for Sentry. Target retention is about 30 days. Uncheck anytime to
          withdraw; new reports stop immediately. Storage key:{' '}
          <code>{DIAGNOSTICS_CONSENT_STORAGE}</code>. Hosted upload remains inactive until
          StreamPulse activates the diagnostics route — consent alone does not enable product
          analytics.
        </p>
      </section>
