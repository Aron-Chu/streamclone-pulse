...
describe('AnalyticsChart position rail', () => {
  it('hides the overview rail on short streams that are fully zoomed out', () => {
    const { container } = renderChart(8)
    expect(container.querySelector('[data-chart-position-rail="true"]')).toBeNull()
  })

  it('shows the overview rail for streams of 90 minutes or longer', () => {
    ...
    expect(container.querySelector('[data-chart-rail-thumb]')?.getAttribute('style')).toContain('rgba(52, 211, 153')
