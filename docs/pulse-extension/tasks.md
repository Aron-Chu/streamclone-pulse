...
- [~] **RPR-1** Request contract + performance (R14): request matrix tests; chart migration v2; content-owned poller + `pulseGetCoordinator` coalesce; Full gated behind explicit load; soft stale refresh; route sync maxWait; bundle budgets; Vite content rebuild on `closeBundle`. _Prior evidence (not full acceptance):_ master `cf2ef08` (PR #46); force-full CI `30138875909`. Reopened for acceptance-repair criteria (activation-scoped Full, backfill cancellation, real request routing, bundle-budget CI, both-host E2E).
- [~] **RPR-2** Manifest targets (dev/CWS/Edge/Firefox-later) without localhost on store builds (R18); yauzl ZIP byte validation; store DCE; exact permission allowlists; distinct CWS/Edge ZIP names; portal localhost scanners; RPR-6 readiness note for sibling `file:` deps. _Prior evidence (not full acceptance):_ master `cf2ef08` (PRs #44/#47); force-full CI `30138875909`. Reopened for package-policy acceptance repairs. Do **not** upload store ZIPs yet.
