...
export function useCollectionArrival(root: RefObject<HTMLElement | null>, identitySignature: string) {
  ...
  useLayoutEffect(() => {
    ...
    root.current?.querySelectorAll<HTMLElement>('[data-arrival-key]').forEach(element => {
      ...
      // Result rows contain links and buttons as soon as they mount. Keep their
      // text fully opaque throughout the entrance so muted labels retain their
      // contrast and keyboard focus never lands on an invisible control.
